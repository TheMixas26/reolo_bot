            ...
            for event in vk_adapter.listen():
                ...
                if parsed.route != "post":
                    if parsed.route == "message":
                        vk_adapter.send_message(peer_id, "Тег #message из VK пока не поддерживается. Напиши напрямую админу в Telegram.")
                    elif parsed.route == "report":
                        vk_adapter.send_message(peer_id, "Тег #report из VK пока не поддерживается отдельным маршрутом. Лучше продублировать это в Telegram.")
                    elif parsed.route == "event":
                        vk_adapter.send_message(peer_id, "Тег #event из VK пока не поддерживается отдельным маршрутом. Лучше прислать идею в Telegram.")
                    continue
