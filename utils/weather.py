...
class WeatherService:
    ...
    WEATHER_CODES = {
        ...
        (71, 73, 75): "❄️",  # Снег
        (77): "🌨️",  # Снежные зерна
        (80, 81, 82): "⛈️",  # Ливни   # ливни... из катки типо?))))
...
def send_weather():
    ...
    try:
        ...
        if not forecast:
            error_msg = "❌ Ошибка при получении прогноза погоды. Проверьте подключение к интернету и корректность координат." # то есть блять без интернета сообщение в тг смoжет отправиться, да?)))
            if admin:
