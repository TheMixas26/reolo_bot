...
def start_command(message):
    ...
    rpg_bot.reply_to(message, "Добро пожаловать в карточную игру! Используйте /help, чтобы посмотреть команды.")
...
def cancel_command(message):
    ...
    rpg_bot.reply_to(message, "Ваше текущее карточное действие отменено.")
...
def roll_command(message):
    ...
    if not packs:
        rpg_bot.reply_to(message, "Паки пока не настроены.")
        return
...
def wallet_command(message):
    ...
    rpg_bot.send_message(message.chat.id, format_wallet(balance), parse_mode="HTML")
...
def events_command(message):
    ...
    rpg_bot.send_message(message.chat.id, format_event_list(events), parse_mode="HTML")
...
def inventory_command(message):
    ...
    if not inventory:
        rpg_bot.reply_to(message, "Ваш инвентарь пуст. Откройте первый пак через /roll.")
        return
    rpg_bot.send_message(message.chat.id, format_inventory(inventory), parse_mode="HTML")
...
def cards_command(message):
    ...
    cards = get_all_cards()
    rpg_bot.send_message(message.chat.id, format_card_catalog(cards), parse_mode="HTML")
...
def duel_command(message):
    ...
    if opponent is None:
        rpg_bot.reply_to(message, "Для дуэли ответьте командой /duel на сообщение другого игрока.")
        return
    ...
    if not challenger_inventory:
        rpg_bot.reply_to(message, "У вас нет карт для дуэли. Откройте пак через /roll.")
        return
    if not opponent_inventory:
        rpg_bot.reply_to(message, "У второго игрока пока нет карт для дуэли.")
        return
    if _user_busy(message.from_user.id) or _user_busy(opponent.id):
        rpg_bot.reply_to(message, "Один из игроков уже занят другим карточным действием.")
        return

    placeholder = rpg_bot.send_message(message.chat.id, "Создаю дуэльный вызов...")
...
def team_battle_command(message):
    ...
    opponent = _reply_target(message)
    if opponent is None:
        rpg_bot.reply_to(message, "Для командного боя ответьте командой /team_battle на сообщение другого игрока.")
        return
    ...
    if count_total_inventory_cards(challenger_inventory) < TEAM_SIZE:
        rpg_bot.reply_to(message, f"Для командного боя нужно минимум {TEAM_SIZE} карт в инвентаре.")
        return
    if count_total_inventory_cards(opponent_inventory) < TEAM_SIZE:
        rpg_bot.reply_to(message, "У второго игрока пока недостаточно карт для командного боя.")
        return
    if _user_busy(message.from_user.id) or _user_busy(opponent.id):
        rpg_bot.reply_to(message, "Один из игроков уже занят другим карточным действием.")
        return

    placeholder = rpg_bot.send_message(message.chat.id, "Создаю командный вызов...")
    lobby = ChallengeLobby(
        mode="team",
        chat_id=placeholder.chat.id,
        message_id=placeholder.message_id,
...
def events_admin_command(message):
    ...
    rpg_bot.reply_to(message, format_admin_event_list(get_card_events()), parse_mode="HTML")
...
def create_event_command(message):
    ...
    if " " not in message.text:
        rpg_bot.reply_to(message, "Введите данные ивента в формате:\nНазвание | Награда | Описание")
...
def _create_event_from_payload(message, payload: str):
    try:
        ...
        rpg_bot.reply_to(message, f"Карточный ивент создан: #{event_id} {title}")
        log_event(
            "card_event_created",
            bot="rpg",
            user_id=message.from_user.id,
        ...
        rpg_bot.reply_to(message, "Формат: /cg_create_event Название | Награда | Описание")
...
def reward_event_command(message):
    ...
    if target is None:
        rpg_bot.reply_to(message, "Ответьте этой командой на сообщение участника: /cg_reward_event ID_ивента")
        return
    ...
    try:
        ...
        if granted:
            rpg_bot.reply_to(message, f"Игрок получил {reward} Имперских Батов за участие в ивенте #{event_id}.")
            log_event(
            ...
            rpg_bot.reply_to(message, "Этому игроку награда за данный ивент уже выдавалась.")
    except ValueError as error:
        rpg_bot.reply_to(message, f"Ошибка: {error}")
...
def packs_admin_command(message):
    ...
    rpg_bot.reply_to(message, format_admin_pack_list(get_all_packs()))
...
def add_pack_command(message):
    ...
    try:
        ...
        rpg_bot.reply_to(message, f"Пак «{name}» создан или обновлён.")
        log_event(
            "pack_upserted",
            bot="rpg",
            user_id=message.from_user.id,
        ...
        rpg_bot.reply_to(message, "Формат: /cg_add_pack Название | Цена | Описание")
...
def edit_pack_command(message):
    ...
    try:
        ...
        rpg_bot.reply_to(message, f"Пак #{pack_id} обновлён.")
        log_event(
            "pack_updated",
            bot="rpg",
            user_id=message.from_user.id,
        ...
        rpg_bot.reply_to(message, f"Формат: /cg_edit_pack ID | price=50 | description=... | is_active=1 | name=...\nОшибка: {error}")
...
def add_card_command(message):
    ...
    try:
        ...
        if get_pack_by_name(card_data["category"]) is None:
            rpg_bot.reply_to(message, "Сначала создайте пак для этой категории через /cg_add_pack.")
            return
        card_id = add_card(card_data)
        rpg_bot.reply_to(message, f"Карта добавлена с id #{card_id}.")
        log_event(
            "card_created",
            bot="rpg",
            user_id=message.from_user.id,
...
def edit_card_command(message):
    ...
    try:
        ...
        if "category" in updates and updates["category"] and get_pack_by_name(updates["category"]) is None:
            rpg_bot.reply_to(message, "Указанный пак не существует. Сначала создайте его через /cg_add_pack.")
            return
        update_card(card_id, updates)
        rpg_bot.reply_to(message, f"Карта #{card_id} обновлена.")
        log_event(
            "card_updated",
            bot="rpg",
            user_id=message.from_user.id,
